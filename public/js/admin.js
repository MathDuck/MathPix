...
// Cache optimisation (id -> { saved_pct, final_bytes })
...
function renderDashboardRecent(list) {
    ...
    list.forEach(img => {
        ...
        row.innerHTML = `<div class="recent-cell-id">${img.id}${apiBadge}</div><div class="recent-cell-user">${uname}${img.owner_id ? `<br><span class='muted'>${img.owner_id}</span>` : ''}</div><div class="recent-cell-date" data-optim-target="${img.id}">${created}</div><div class="recent-cell-actions"><button type="button" class="btn btn-secondary btn-xs" data-act="open">Voir</button><a href="${img.url}" class="btn btn-primary btn-xs" target="_blank" rel="noopener" data-act="direct">Ouvrir</a></div>`;
        row.addEventListener('click', e => { if (e.target.getAttribute && e.target.getAttribute('data-act') === 'open') openLightbox({ id: img.id, url: img.url, ext: img.ext, size: img.size, original_name: img.original_name }); });
...
function renderDashboardTopIps(list) {
    ...
    list.forEach(ip => {
        ...
        tr.innerHTML = `<td>${ip.ip}</td><td>${ip.score}</td>`;
        frag.appendChild(tr);
    });
...
async function loadAllImages() {
    ...
    (data.images || []).forEach(imageRow => {
        const imageUrl = `${location.origin}/i/${imageRow.id}${imageRow.ext}`;
        ...
        tr.innerHTML = `<td data-optim-target="${imageRow.id}">${imageRow.id}${apiBadge}</td><td>${imageRow.owner_id ? `${imageRow.owner_id}<br><span class='muted'>${ownerUsername}</span>` : '-'}</td><td>${new Date(imageRow.created_at * 1000).toLocaleString()}</td><td><div class=\"btn-group-compact\"><button class=\"btn btn-secondary btn-xs view\" data-id=\"${imageRow.id}\" data-url=\"${imageUrl}\" data-ext=\"${imageRow.ext}\" data-size=\"${imageRow.size || 0}\" data-original-name=\"${(imageRow.original_name || '').replace(/"/g, '&quot;')}\">Voir</button><button class=\"btn btn-error btn-xs del\" data-id=\"${imageRow.id}\">Suppr.</button></div></td>`;
        tbody.appendChild(tr);
    ...
    tbody.querySelectorAll('.view').forEach(btn => btn.addEventListener('click', e => {
        ...
        openLightbox({ id: button.getAttribute('data-id'), url: button.getAttribute('data-url'), ext: button.getAttribute('data-ext'), size: parseInt(button.getAttribute('data-size') || '0', 10), original_name: button.getAttribute('data-original-name') || undefined });
    }));
...
async function loadLogs() {
    ...
    if (!list.length) {
        ...
        list.forEach(l => {
            ...
            const userCell = (l.user_id || uname) ? `${l.user_id || ''}${uname ? `<br><span class='muted'>${uname}</span>` : ''}` : '';
